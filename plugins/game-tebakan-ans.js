...
export async function before(m) {
    ...
    if (!(id in this.tebaktebakan))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, buttontebaktebakan, m)
    if (m.quoted.id == this.tebaktebakan[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebaktebakan[id][3])
            delete this.tebaktebakan[id]
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', wm, null, buttontebaktebakan, m)
        }
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.tebaktebakan[id][2]} XP`, wm, null, buttontebaktebakan, m)
            clearTimeout(this.tebaktebakan[id][3])
            delete this.tebaktebakan[id]
