...
`.trim()
    ...
    let res = await fetch(`https://api.lolhuman.xyz/api/mediafire?apikey=${set.lolkey}&url=${args[0]}`)
...
*📨 uploaded:* ${x.result.uploaded}
    `
    conn.sendButtonImg(m.chat, set.logo, caption, wm, 'G E T', '.get ' + x.result.link, fakes, adReply)
    }
}
