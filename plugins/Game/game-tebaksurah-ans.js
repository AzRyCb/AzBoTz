...
export async function before(m) {
    ...
    if (!(id in this.tebaksurah))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, buttontebaksurah, m)
    if (m.quoted.id == this.tebaksurah[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebaksurah[id][3])
            delete this.tebaksurah[id]
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', wm, null, buttontebaksurah, m)
        }
        ...
        if (m.text.toLowerCase() == json.surah.englishName.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.tebaksurah[id][2]} XP`, wm, null, buttontebaksurah, m)
            clearTimeout(this.tebaksurah[id][3])
            delete this.tebaksurah[id]
