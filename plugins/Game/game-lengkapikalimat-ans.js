...
export async function before(m) {
    ...
    if (!(id in this.lengkapikalimat))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, buttonlengkapikalimat, m)
    if (m.quoted.id == this.lengkapikalimat[id][0].id) {
        ...
        if (isSurrender) {
            ...
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', wm, null, buttonlengkapikalimat, m)
        }
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.lengkapikalimat[id][2]} XP`, wm, null, buttonlengkapikalimat, m)
            clearTimeout(this.lengkapikalimat[id][3])
            delete this.lengkapikalimat[id]
