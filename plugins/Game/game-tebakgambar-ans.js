...
export async function before(m) {
    ...
    if (!(id in this.tebakingambar))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, buttontebakingambar, m)
    if (m.quoted.id == this.tebakingambar[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebakingambar[id][3])
            delete this.tebakingambar[id]
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', wm, null, buttontebakingambar, m)
        }
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.tebakingambar[id][2]} XP`, wm, null, buttontebakingambar, m)
            clearTimeout(this.tebakingambar[id][3])
            delete this.tebakingambar[id]
