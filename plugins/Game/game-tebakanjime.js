...
let handler = async (m, { conn, command, usedPrefix }) => {
    ...
    if (id in conn.tebakanjime) {
        conn.sendButton(m.chat, 'Masih ada soal belum terjawab di chat ini', wm, null, buttons, conn.tebakanjime[id][0])
        throw false
    }
...
Bonus: ${poin} XP
    `.trim()
    conn.tebakanjime[id] = [
        await conn.sendButton(m.chat, caption, wm, json.image, buttons, m),
        json, poin,
        setTimeout(() => {
            if (conn.tebakanjime[id]) conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.name}*\n*Desk:* ${json.desc}`, wm, null, [
