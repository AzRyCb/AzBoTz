...
let handler = async (m, { conn, command, usedPrefix }) => {
    ...
    if (id in conn.tebakchara) {
        conn.sendButton(m.chat, 'Masih ada soal belum terjawab di chat ini', wm, null, buttons, conn.tebakchara[id][0])
        throw false
    }
...
Bonus: ${poin} XP
    `.trim()
    conn.tebakchara[id] = [
        await conn.sendButton(m.chat, caption, wm, `${json.images.jpg.image_url}`, buttons, m),
        json, poin,
        setTimeout(() => {
            if (conn.tebakchara[id]) conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.name}*\nKanji : ${json.name_kanji}\n*Url :* ${json.url}\n*Desk :* ${json.about}`, wm, json.images.jpg.image_url, [
