...
let handler = async (m, { conn, usedPrefix: _p, args, command, __dirname }) => {
  ...
  let arrayMenu = ['all', 'misc', 'store', 'user', 'edukasi', 'lagu', 'anonymous', 'ephoto360', 'baileys', 'database', 'nocategory', 'primbon', 'canvas', 'absen', 'update','rpg', 'anime', 'virus', 'downloader', 'game', 'fun', 'xp', 'news', 'github', 'group', 'image', 'admin', 'info', 'internet', 'islam', 'kerang', 'maker', 'owner', 'audio', 'premium', 'quotes', 'stalk', 'shortlink', 'sticker', 'tools', 'nsfw', 'asupan', 'random', 'textpro', 'photooxy']
...
╰――————————————❑`
   const sections = [
    ...
    title: `––––––『 MENU 』––––––`,
     ...
     {title: `🎨 │ Stikers`, rowId: `${_p}? stiker`, description: "Buat Sticker diBOT"},
     {title: `🔝 │ Github`, rowId: `${_p}? github`, description: "Download Sesuatu Dari Github"},
...
title: `––––––『 Maker 』––––––`,
 rows: [
  ...
  sections
    ...
    return conn.sendMessage(m.chat, listMessage, { quoted: m, contextInfo:{ mentionedJid: [m.sender] }})
    //return conn.sendMessage(m.chat, listMessage, m)
    //bang bang gimana biar fkontak nya bisa <emotebatu>
  }    
    ...
    let replace = {
      ...
      level, limit, platform, mode, prems, money, name, wib, wit, wita, weton, week, date, dateIslamic, time, totalreg, rtotalreg, role,
      readmore: readMore
    }
    text = text.replace(new RegExp(`%(${Object.keys(replace).sort((a, b) => b.length - a.length).join`|`})`, 'g'), (_, name) => '' + replace[name])
    const pp = await conn.profilePictureUrl(conn.user.jid).catch(_ => './src/avatar_contact.png')
    conn.sendHydrated(m.chat, text.trim(), wm, pp, set.web, 'Web', null, null, [
      ['Donate', '/donasi'],
      ['Speed', '/ping'],
      ['Owner', '/owner']
    ], m)
        ...
        //await conn.send3TemplateButtonImg(m.chat, await(await fetch(set.fla+ teks)).buffer(), text.trim(), `Aktif selama : ${uptime}`, `🏅Owner`, `${_p}owner`, `🎖ThanksTo`, `${_p}tqto`, `🎗  Donasi  🎗`, `${_p}infobot`)
