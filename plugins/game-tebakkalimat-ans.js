...
export async function before(m) {
    ...
    if (!(id in this.tebakkalimat))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, buttontebakkalimat, m)
    if (m.quoted.id == this.tebakkalimat[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebakkalimat[id][3])
            delete this.tebakkalimat[id]
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', wm, null, buttontebakkalimat, m)
        }
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.tebakkalimat[id][2]} XP`, wm, null, buttontebakkalimat, m)
            clearTimeout(this.tebakkalimat[id][3])
            delete this.tebakkalimat[id]
