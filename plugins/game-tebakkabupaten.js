...
let handler = async (m, { conn, command, usedPrefix }) => {
    ...
    if (id in conn.tebakkabupaten) {
        conn.sendButton(m.chat, 'Masih ada soal belum terjawab di chat ini', wm, null, buttons, conn.tebakkabupaten[id][0])
        throw false
    }
...
Bonus: ${poin} XP
    ...
    conn.tebakkabupaten[id] = [
        await conn.sendButton(m.chat, caption, wm, json.url, buttons, m),
        json, poin,
        setTimeout(() => {
            if (conn.tebakkabupaten[id]) conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.title}*`, wm, null, [
