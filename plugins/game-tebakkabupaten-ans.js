...
export async function before(m) {
    ...
    if (!(id in this.tebakkabupaten))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, buttontebakkabupaten, m)
    if (m.quoted.id == this.tebakkabupaten[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebakkabupaten[id][3])
            delete this.tebakkabupaten[id]
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', wm, null, buttontebakkabupaten, m)
        }
        ...
        if (m.text.toLowerCase() == json.title.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.tebakkabupaten[id][2]} XP`, wm, null, buttontebakkabupaten, m)
            clearTimeout(this.tebakkabupaten[id][3])
            delete this.tebakkabupaten[id]
