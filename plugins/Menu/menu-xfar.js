...
*total_episode:* ${_ff.total_episode}`
        conn.sendButton(m.chat, fff, wm, await(await fetch(_ff.thumbnail)).buffer(), [[' Menu', '/menu']], m, adReply)
            break
...
*no_watermark:* ${_hh.no_watermark}`
            conn.sendButton(m.chat, hhh, wm, await(await fetch(_hh.thumbnail)).buffer(), [['NoWm', '/get ' + _hh.no_watermark], ['Wm', '/get ' + _hh.watermark]], m, adReply)
            break
            case 'facebook':
...
*sd:* ${_ii.sd}
*hd:* ${_ii.hd}`
conn.sendButton(m.chat, iii, wm, logo, [['SD', '/get ' + _ii.sd], ['HD', '/get ' + _ii.hd]], m, adReply)
            break
            case 'imdb':
        let jj = await xa.downloader.imdb(one)
        let _jj = jj
            let jjj = `${_jj.res}`
conn.sendButton(m.chat, jjj, wm, logo, [['Get', '/get ' + _jj]], m, adReply)
            break
            case 'imgur':
...
*img:* ${_kk.image_link}`
conn.sendButton(m.chat, kkk, wm, logo, [['Vid', '/get ' + _kk.video_link], ['Img', '/get ' + _kk.image_link]], m, adReply)
            break
            case 'instagram':
...
*media:* ${_ll.media}`
            conn.sendButton(m.chat, lll, wm, await(await fetch(_ll.thumbnail)).buffer(), [['Vid', '/get ' + _ll.media]], m, adReply)
            break
            case 'like':
...
*no_watermark:* ${_mm.no_watermark}`
            conn.sendButton(m.chat, mmm, wm, await(await fetch(_mm.thumbnail)).buffer(), [['NoWm', '/get ' + _mm.no_watermark], ['Wm', '/get ' + _mm.watermark]], m, adReply)
            break
            case 'linesticker':
	...
	return conn.sendList(m.chat, 'Select Your Sticker', '!No Spam', wm, 'Here', U, m)
            break
            case 'pinterestdl':
...
*download:* ${_pP.download}`
            conn.sendButton(m.chat, ppP, wm, await(await fetch(_pP.thumbnail)).buffer(), [['Get', '/get ' + _pP.download]], m, adReply)
            break
            case 'telesticker':
...
*media:* ${_rr.media}`
            conn.sendButton(m.chat, rrr, wm, await(await fetch(_rr.thumbnail)).buffer(), [['Vid', '/get ' + _rr.media]], m, adReply)
            break
            case 'twitter':
...
`
            conn.sendButton(m.chat, ttt, wm, await(await fetch(_tt.thumbnail)).buffer(), [['Vid', '/get ' + _tt.download_url]], m, adReply)
            break
            case 'inews':
...
${_BB.pp_picture}
${_BB.about}`
conn.sendButton(m.chat, BBB, wm ,logo, [['PP', '/get ' + _BB.pp_picture]], m, adReply)
            break
            case 'listsurah':
...
${v.source}`,
	...
	return conn.sendListM(m.chat, EEn_, EE_, m)
        ...
        let _FF = FF
            let FFi = `*title:* ${one}`
conn.sendButton(m.chat, FFi, wm, _FF.result.result.resp, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'ttp':
        let GG = await xa.maker.ttp(one)
        let _GG = GG
            let GGi = `*title:* ${one}`
conn.sendButton(m.chat, GGi, wm, _GG.result.result.resp, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'bingimage':
        let HH = await xa.search.bingimage(one)
        let _HH = HH
            let HHi = `*title:* ${_HH.result.resp}`
conn.sendButton(m.chat, HHi, wm, _HH.result, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'bingsearch':
        let II = await xa.search.bingsearch(one)
        let _II = II
            let IIi = `*title:* ${_II.result.resp}`
conn.sendButton(m.chat, IIi, wm, _II.result, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'chord':
...
thumbnail: ${v.thumbnail}`,
	...
	return conn.sendListM(m.chat, NNn_, NN_, m)
        ...
        let _OO = OO
            let OOi = `*title:* ${one}`
conn.sendButton(m.chat, OOi, wm, _OO.url, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'stickersearch':
...
`,
  ...
  if (/webp/g.test(b__a)) d__a = await webp2png(c__a)
        ...
        let _SS = SS
        conn.sendButtonImg(m.chat, _SS.data, wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'whatimage':
  ...
  if (/webp/g.test(b__b)) d__b = await webp2png(c__b)
        ...
        let _TT = TT
            conn.sendButtonImg(m.chat, _TT.data, wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'whatmusic':
            let wmus = await q.download()
        let UU = await xa.search.whatmusic(wmus)
        let _UU = UU
            conn.sendButtonImg(m.chat, _UU.metadata, wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'debase64':
