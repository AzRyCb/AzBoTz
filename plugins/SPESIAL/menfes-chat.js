...
Contoh Penggunaan: ${usedPrefix + command} ${set.author} pesan untuknya

Contoh: ${usedPrefix + command} ${set.author} hai`
let suks = `Mengirim Pesan *${mime ? mime : 'Teks'}*
👥 Dari : @${m.sender.replace(/@.+/, '')}
👥 Untuk : @${mention.replace(/@.+/, '')}

––––––『 💌 Pesan 』––––––
⫹⫺ ${txt ? txt : 'Pesan Kosong'}
`
    // Batas
    ...
    switch (command) {
        case 'menfesleave': {
            ...
            if (!room) return this.sendButton(m.chat, '*Kamu tidak sedang berada di menfes chat*', wm, null, [['Mulai Menfes', '.menfesstart']], null)
            m.reply('Sukses Hapus Menfes')
            let other = room.other(m.sender)
            if (other) await this.sendButton(other, room.b + ' *Meninggalkan chat*', wm, null, [['Mulai Menfes', '.menfesstart']], null)
            delete this.menfes[room.id]
            if (command === 'menfesleave') break
        }
        case 'menfesstart': {
            if (Object.values(this.menfes).find(room => room.check(m.sender))) return this.sendButton(m.chat, '*Kamu masih berada di dalam menfes chat, menunggu Balasan*', wm, null, [['Hapus Menfes', '.menfesleave']], null)
            let room = Object.values(this.menfes).find(room => room.state === 'WAITING' && !room.check(m.sender))
            if (room) {
                room.b = m.sender
                room.state = 'CHATTING'
                await this.sendButton(room.a, '*Menfes Chat Tersambung!*\nDengan: ' + m.sender, wm, null, [['Hapus Menfes', '.menfesleave']], null)
                await this.sendButton(m.sender, '*Menfes Chat Tersambung!*\nDengan: ' + room.a, wm, null, [['Hapus Menfes', '.menfesleave']], null)
            } else {
            // Batas
	if (!m.quoted) {
	...
	await conn.sendButton(m.chat, suks, wm, null, [['Tes Bot', 'tes']], m, { mentions: conn.parseMention(suks) })
            // Batas
                ...
                await this.sendButton(m.chat, '*Menunggu Balasan...*', wm, null, [['Hapus Menfes', '.menfesleave']], null)
            }
            break
        }
    }
