...
export async function before(m) {
    ...
    if (!(id in this.tebaklogo))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, buttontebaklogo, m)
    if (m.quoted.id == this.tebaklogo[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebaklogo[id][3])
            delete this.tebaklogo[id]
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', wm, null, buttontebaklogo, m)
        }
        ...
        if (m.text.toLowerCase() == json.hasil.data.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.tebaklogo[id][2]} XP`, wm, null, buttontebaklogo, m)
            clearTimeout(this.tebaklogo[id][3])
            delete this.tebaklogo[id]
