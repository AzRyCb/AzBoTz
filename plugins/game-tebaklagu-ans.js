...
export async function before(m) {
    ...
    if (!(id in this.tebaklagu))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, buttontebaklagu, m)
    if (m.quoted.id == this.tebaklagu[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebaklagu[id][3])
            delete this.tebaklagu[id]
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', wm, null, buttontebaklagu, m)
        }
        ...
        if (m.text.toLowerCase() == json.judul.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.tebaklagu[id][2]} XP`, wm, null, buttontebaklagu, m)
            clearTimeout(this.tebaklagu[id][3])
            delete this.tebaklagu[id]
