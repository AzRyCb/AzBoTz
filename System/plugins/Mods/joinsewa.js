...
let handler = async (m, { conn, text, usedPrefix, args, participants }) => {
  ...
  let anubot = set.nomorown
  conn.reply(m.chat,`Tunggu 3 detik bot akan join`)
  await delay(3000)
  try {
...
Thanks dah invite Gua *${m.name}*`, fkonn, {
    mentions: d
     ...
     await conn.reply(set.owner, `*INVITING!*\n\n@${m.sender.split('@')[0]} telah mengundang ${conn.user.name} ke grup\n\n${await conn.getName(res)}\n\n${res}\n\nPesan : ${args[0]}\n\nBot akan keluar otomatis setelah *${msToDate(db.data.chats[res].expired - now)}*`, null, {mentions: [m.sender]})
     })
     if (!e.length) await conn.reply(set.owner, `*INVITING!*\n\n@${m.sender.split('@')[0]} telah mengundang ${conn.user.name} ke grup\n\n${await conn.getName(res)}\n\n${res}\n\nPesan : ${args[0]}\n\nBot akan keluar otomatis setelah *${msToDate(db.data.chats[res].expired - now)}*`, null, {mentions: [m.sender]})
...
@${conn.user.jid.split('@')[0]} akan keluar secara otomatis setelah *${msToDate(db.data.chats[res].expired - now)}*`
  await conn.sendButton(res, mes, wm, null, [[`Owner`, `.owner`], [`Menu`, `${usedPrefix}menu`]], fkonn, {
        mentions: d
         })
     })
    } catch (e) {
