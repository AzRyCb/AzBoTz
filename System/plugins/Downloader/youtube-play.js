...
let handler = async (m, { conn, text, args, usedPrefix, command }) => {
	...
	if (text.includes('http://') || text.includes('https://')) {
		...
		try {
			...
			conn.sendButton(m.chat, ini_txt, set.wm, anu.video[0].thumbnail.split("?")[0], [
			...
			try {
				...
				ini_txt += `🌀 *Url :* ${args[0]}`
				conn.sendButton(m.chat, ini_txt, set.wm, anu.thumbnail, [
				...
				console.error(e)
				try {
					let anu2 = await fetch(`https://api.lolhuman.xyz/api/ytvideo?apikey=${set.lolkey}&url=${text}`)
		...
		try {
			...
			conn.sendButton(m.chat, ini_txt, set.wm, anu.video[x].thumbnail.split("?")[0], [
			...
			try {
				...
				conn.sendButton(m.chat, ini_txt, set.wm, anu.result.info.thumbnail, [
