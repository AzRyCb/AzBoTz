...
case 'slice':
        ...
        conn.sendButtonImg(m.chat, nsf, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'gores':
...
${hc.data.username}
    `
	conn.sendButtonVid(m.chat, set.giflogo, he, 'Nih.mp4', 'To Sticker', '.s', fakes, adReply)
            break
            case 'igstory':
         if (!one) throw `Contoh penggunaan ${usedPrefix + command} ${args[0]} |query`
        ...
        let jc = await jb.json()
            conn.sendButtonImg(m.chat, jc.data.url, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'mediafire':
...
${kc.data.uploaded}
    `
	conn.sendButtonVid(m.chat, set.giflogo, ke, 'Nih', 'Get', '.get ' + kc.data.link, fakes, adReply)
            break
            case 'pin':
         if (!one) throw `Contoh penggunaan ${usedPrefix + command} ${args[0]} |query`
...
${nc.data.title}
    `
	conn.sendButtonVid(m.chat, set.giflogo, ne, 'Nih', 'Get', '.get ' + nc.data.audio, fakes, adReply)
            break
            case 'rexdlget':
...
${oc.data.url}
	...
	return conn.sendListM(m.chat, pf, pe, m)
        ...
        conn.reply(m.chat, 'Result: ' + qc.data.url)
                break
                case 'soundclouds':
...
${sc.data.author.verified}`
	conn.sendButtonVid(m.chat, sc.data.imageURL, se, 'Nih', 'Get', '.get ' + sc.data.url, fakes, adReply)
        ...
        let tb = `https://api.neoxr.my.id/api/ss?url=${one}&apikey=5VC9rvNx`
            conn.sendButtonImg(m.chat, tb, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'sshp':
         if (!one) throw `Contoh penggunaan ${usedPrefix + command} ${args[0]} |query`
        let ub = `https://api.neoxr.my.id/api/sshp?url=${one}&apikey=5VC9rvNx`
            conn.sendButtonImg(m.chat, ub, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
...
${Ac.data.caption}`
	conn.send2ButtonVid(m.chat, Ac.data.videoset.wm, Ae, 'Nih', 'No set.wm', '.get ' + Ac.data.video, 'MP3', '.get ' + Ac.data.audio, fakes, adReply)
            break
...
${Bc.downloads[2].type}
${Bc.downloads[2].url}`
	conn.send2ButtonVid(m.chat, Bc.downloads.url, Be, 'Nih', 'No set.wm', '.get ' + Bc.downloads[1].url, 'MP3', '.get ' + Bc.downloads[2].url, fakes, adReply)
            break
            case 'twitter':
