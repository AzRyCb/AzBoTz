...
switch (template) {
	...
	return await conn.sendListM(m.chat, bf, be, m)
        ...
        conn.send2ButtonVid(m.chat, cc.video, cc.title, cc.author, `set.wm`, `.get ${cc.videoset.wm}`, `Audio`, `.get ${cc.audio}`, fakes, adReply)
            break
            case 'tiktok2':
        let cb2 = await fetch(`https://malesin.xyz/tiktok2?url=${one}`)
        let cc2 = await cb2.json()
        conn.send2ButtonVid(m.chat, giflogo, set.wm, `Ori`, `.get ${cc2.audio_original}`, `Audio`, `.get ${cc2.audio}`, fakes, adReply)
            break
           case 'pindl':
        let cb3 = await fetch(`https://malesin.xyz/pindl?url=${one}`)
        let cc3 = await cb3.json()
        conn.sendButtonImg(m.chat, cc3.result, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'xnxxdl':
