...
*total_episode:* ${_ff.total_episode}`
        conn.sendButton(m.chat, fff, set.wm, await(await fetch(_ff.thumbnail)).buffer(), [[' Menu', '/menu']], m, adReply)
            break
...
*no_watermark:* ${_hh.no_watermark}`
            conn.sendButton(m.chat, hhh, set.wm, await(await fetch(_hh.thumbnail)).buffer(), [['Noset.wm', '/get ' + _hh.no_watermark], ['set.wm', '/get ' + _hh.watermark]], m, adReply)
            break
            case 'facebook':
...
*sd:* ${_ii.sd}
*hd:* ${_ii.hd}`
conn.sendButton(m.chat, iii, set.wm, set.logobot, [['SD', '/get ' + _ii.sd], ['HD', '/get ' + _ii.hd]], m, adReply)
            break
            case 'imdb':
        let jj = await xa.downloader.imdb(one)
        let _jj = jj
            let jjj = `${_jj.res}`
conn.sendButton(m.chat, jjj, set.wm, set.logobot, [['Get', '/get ' + _jj]], m, adReply)
            break
            case 'imgur':
...
*img:* ${_kk.image_link}`
conn.sendButton(m.chat, kkk, set.wm, set.logobot, [['Vid', '/get ' + _kk.video_link], ['Img', '/get ' + _kk.image_link]], m, adReply)
            break
            case 'instagram':
...
*media:* ${_ll.media}`
            conn.sendButton(m.chat, lll, set.wm, await(await fetch(_ll.thumbnail)).buffer(), [['Vid', '/get ' + _ll.media]], m, adReply)
            break
            case 'like':
...
*no_watermark:* ${_mm.no_watermark}`
            conn.sendButton(m.chat, mmm, set.wm, await(await fetch(_mm.thumbnail)).buffer(), [['Noset.wm', '/get ' + _mm.no_watermark], ['set.wm', '/get ' + _mm.watermark]], m, adReply)
            break
            case 'linesticker':
	...
	return conn.sendList(m.chat, 'Select Your Sticker', '!No Spam', set.wm, 'Here', U, m)
            break
            case 'pinterestdl':
...
*download:* ${_pP.download}`
            conn.sendButton(m.chat, ppP, set.wm, await(await fetch(_pP.thumbnail)).buffer(), [['Get', '/get ' + _pP.download]], m, adReply)
            break
            case 'telesticker':
...
*media:* ${_rr.media}`
            conn.sendButton(m.chat, rrr, set.wm, await(await fetch(_rr.thumbnail)).buffer(), [['Vid', '/get ' + _rr.media]], m, adReply)
            break
            case 'twitter':
...
const ssx = {
  text: ' ',
  footer: set.botdate,
  title: `*${set.htki} RESULT ${set.htka}*\n${_ss.nickname}\n${_ss.caption}\n${_ss.thumbnail}`,
...
`
            conn.sendButton(m.chat, ttt, set.wm, await(await fetch(_tt.thumbnail)).buffer(), [['Vid', '/get ' + _tt.download_url]], m, adReply)
            break
            case 'inews':
...
${_BB.pp_picture}
${_BB.about}`
conn.sendButton(m.chat, BBB, set.wm ,set.logobot, [['PP', '/get ' + _BB.pp_picture]], m, adReply)
            break
            case 'listsurah':
        let CC = await xa.islami.listsurah()
        let _CC = CC
        conn.reply(m.chat, _CC.listsurah)
            break
            case 'surah':
...
${v.source}`,
	...
	return conn.sendListM(m.chat, EEn_, EE_, m)
        ...
        let _FF = FF
            let FFi = `*title:* ${one}`
conn.sendButton(m.chat, FFi, set.wm, _FF.result.result.resp, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'ttp':
        let GG = await xa.maker.ttp(one)
        let _GG = GG
            let GGi = `*title:* ${one}`
conn.sendButton(m.chat, GGi, set.wm, _GG.result.result.resp, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'bingimage':
        let HH = await xa.search.bingimage(one)
        let _HH = HH
            let HHi = `*title:* ${_HH.result.resp}`
conn.sendButton(m.chat, HHi, set.wm, _HH.result, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'bingsearch':
        let II = await xa.search.bingsearch(one)
        let _II = II
            let IIi = `*title:* ${_II.result.resp}`
conn.sendButton(m.chat, IIi, set.wm, _II.result, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'chord':
        let JJ = await xa.search.chord(one)
        let _JJ = JJ
        conn.reply(m.chat, _JJ.title + '\n' + _JJ.artist + '\n' + _JJ.chord)
            break
            case 'drakor':
...
thumbnail: ${v.thumbnail}`,
	...
	return conn.sendListM(m.chat, NNn_, NN_, m)
        ...
        let _OO = OO
            let OOi = `*title:* ${one}`
conn.sendButton(m.chat, OOi, set.wm, _OO.url, [['Sticker', '/s'], ['Back', '/menu']], m, adReply)
            break
            case 'stickersearch':
...
`,
  ...
  if (/webp/g.test(b__b)) d__b = await webp2png(c__b)
        ...
        let _UU = UU
            conn.sendButtonImg(m.chat, _UU.metadata, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'debase64':
        let VV = await xa.tools.debase64(one)
        let _VV = VV
        conn.reply(m.chat, _VV.result)
            break
            case 'debinary':
        let WW = await xa.tools.debinary(one)
        let _WW = WW
            conn.reply(m.chat, _WW.result)
            break
            case 'deobfuscatorjs':
        let XX = await xa.tools.deobfuscatorjs(one)
        let _XX = XX
            conn.reply(m.chat, _XX.result)
            break
            case 'ebase64':
        let YY = await xa.tools.ebase64(one)
        let _YY = YY
            conn.reply(m.chat, _YY.result)
            break
            case 'ebinary':
        let ZZ = await xa.tools.ebinary(one)
        let _ZZ = ZZ
            conn.reply(m.chat, _ZZ.result)
            break
            case 'obfuscatorjs':
