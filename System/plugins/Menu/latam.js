...
${v.video_2}`]
	...
	return conn.sendList(m.chat, '––––––『 📺 HENTAI VIDEO 🔎 』––––––' , `⚡ Silakan pilih HENTAI VIDEO di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`, set.wm, `☂️ HENTAI VIDEO Disini ☂️`, listSections, m)
            case 'hentai':
case 'neko_h':
case 'waifu_h':
        let hen = await fetch(`https://latam-api.vercel.app/api/${args[0]}?apikey=nekosmic`)
        let tai = await hen.json()
        conn.sendButtonImg(m.chat, tai.hidrogeno, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'kuack':
case 'anime_wpp':
case 'dogis':
...
case 'xd_img':
        ...
        conn.sendButtonImg(m.chat, ack.imagen, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            case 'audi_top':
case 'rand_audio':
let kuas = await fetch(`https://latam-api.vercel.app/api/${args[0]}?apikey=nekosmic`)
        ...
        conn.sendButtonImg(m.chat, acxk.randimg, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
            
            case 'txtimg':
        let kuta = await fetch(`https://latam-api.vercel.app/api/txtimg?apikey=nekosmic&q=${one}`)
        let actk = await kuta.json()
        conn.sendButtonImg(m.chat, actk.txtimg, set.wm, 'Nih.jpg', 'To Sticker', '.s', fakes, adReply)
            break
