...
let ps = groupMetadata.participants.map(v => v.id)
  ...
  try {
    ...
    if(db.data.users[m.sender].pasangan != "" && db.data.users[db.data.users[m.sender].pasangan].pasangan == m.sender && db.data.users[m.sender].pasangan != user){
      conn.sendButton(m.chat, `Kamu sudah berpacaran dengan @${db.data.users[m.sender].pasangan.split('@')[0]}\n\nSilahkan putus dulu untuk menembak @${user.split('@')[0]}\n\nBtw yang setia dikit banget!`, '(ketik .putus untuk memutuskan hubungan)', null, ['Putus', '.putus'], m , { contextInfo: { mentionedJid: [user, db.data.users[m.sender].pasangan]}})
