...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      if (/video/g.test(mime)) if ((q.msg || q).seconds > 11) return conn.reply(m.chat, 'Maksimal 10 detik!')
      ...
      if (isUrl(args[0])) stiker = await sticker(false, args[0], set.packname, set.author)
      else return conn.reply(m.chat, 'URL tidak valid!')
    }
  } catch (e) {
    console.error(e)
    if (!stiker) stiker = e
