import daily from './claim-daily.js'
import weekly from './claim-weekly.js'
import monthly from './claim-monthly.js'
...
if (!args[0]) {
	const sections = [
    {
	title: set.htki + 'Inventory' + set.htka,
	rows: [
{title: set.dmenub + "Inventory 1", rowId: usedPrefix + command + ' 1'},
{title: set.dmenub + "Inventory 2", rowId: usedPrefix + command + ' 2'},
{title: set.dmenub + "Inventory 3", rowId: usedPrefix + command + ' 3'},
{title: set.dmenub + "Inventory 4", rowId: usedPrefix + command + ' 4'}
	]
    },{
	title: set.htki + 'Kolam' + set.htka,
	rows: [
{title: set.dmenub + "Kolam 1", rowId: usedPrefix + command + ' invkolam'},
{title: set.dmenub + "Kolam 2", rowId: usedPrefix + 'kolamikan'}
...
const listMessage = {
  text: `${set.htjava} *INVENTORY*
${set.cmenub} *Name:* ${name}
${set.cmenuf}
...
await conn.sendButton(m.chat, str, set.wm, imgr + 'inventory', [[`ᴛʀᴀɴsғᴇʀ`, `.transfer`],
[`ᴀᴅᴠᴇɴᴛᴜʀᴇ`, `.adventure`]], m, { mentions: conn.parseMention(str) })
} else if (args[0] == '2') {
// Inventory 2
let user = user
  const tools = Object.keys(inventory.tools).map(v => user[v] && `*${set.rpg.emoticon(v)}${v}:* ${typeof inventory.tools[v] === 'object' ? inventory.tools[v][user[v]?.toString()] : `Level(s) ${user[v]}`}`).filter(v => v).join('\n').trim()
  const items = Object.keys(inventory.items).map(v => user[v] && `*${set.rpg.emoticon(v)}${v}:* ${user[v]}`).filter(v => v).join('\n').trim()
  const dura = Object.keys(inventory.durabi).map(v => user[v] && `*${set.rpg.emoticon(v)}${v}:* ${user[v]}`).filter(v => v).join('\n').trim()
  const crates = Object.keys(inventory.crates).map(v => user[v] && `*${set.rpg.emoticon(v)}${v}:* ${user[v]}`).filter(v => v).join('\n').trim()
  const pets = Object.keys(inventory.pets).map(v => user[v] && `*${set.rpg.emoticon(v)}${v}:* ${user[v] >= inventory.pets[v] ? 'Max Levels' : `Level(s) ${user[v]}`}`).filter(v => v).join('\n').trim()
...
${Object.keys(inventory.others).map(v => user[v] && `⮕ ${set.rpg.emoticon(v)} ${v}: ${user[v]}`).filter(v => v).join('\n')}${tools ? `
🔖 ᴛᴏᴏʟs :
${tools}` : ''}${items ? `
...
`.trim()
await conn.sendButton(m.chat, caption, set.wm, imgr + 'inventory', [[`ᴛʀᴀɴsғᴇʀ`, `.transfer`],
[`ᴀᴅᴠᴇɴᴛᴜʀᴇ`, `.adventure`]], m, { mentions: conn.parseMention(caption) })
} else if (args[0] == '3') {
// Inventory 3
let date = set.botdate
    let { registered, age, lastrampok, lastdagang, lastcodereg, lastberkebon, lasthourly, lastberburu, lastbansos, lastadventure, lastfishing, lastwar, lastduel, lastmining, lastdungeon, lastclaim, lastweekly, lastmonthly } = user
    let healt = user.healt
    let lapar = user.laper
    let haus = user.haus
    let usrname = user.name
    ...
    let psepick = user.psepick
    let psenjata = user.psenjata

    let ikan= user.ikan
    let nila= user.nila
    let bawal= user.bawal
    let lele= user.lele
    let udangb= user.udang
    ...
    let arlok = user.arlok
    let limit = user.limit
    let glimit = user.glimit
...
╰──────────━⃝┅⃝━━────────┄⸙
`
    await conn.sendButton(m.chat, str, set.botdate, imgr + 'inventory', [[`${healt < 40 ? 'Heal' : 'Adventure'}`, `${healt < 40 ? '.heal' : '.adventure'}`], ['Shop', '.shop']], m, {quoted: fakes})
} else if (args[0] == '4') {
// Inventory 4
...
await conn.sendButton(m.chat, aineh, set.wm, imgr + 'inventory', [[`ᴛʀᴀɴsғᴇʀ`, `.transfer`],
