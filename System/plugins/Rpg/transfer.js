...
async function handler(m, { conn, args, usedPrefix, command }) {
    if (confirmation[m.sender]) return conn.reply(m.chat, 'Kamu sedang melakukan transfer!')
...
`.trim()
    ...
    confirmation[m.sender] = {
        ...
        type,
        count,
        timeout: setTimeout(() => (conn.reply(m.chat, 'Timeout'), delete confirmation[m.sender]), 60 * 1000)
    }
}
...
handler.before = async m => {
    ...
    if (/no?/g.test(m.text.toLowerCase())) {
        ...
        return this.reply(m.chat, 'Reject')
    }
