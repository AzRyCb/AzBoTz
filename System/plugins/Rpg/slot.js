...
let frep = { contextInfo: { externalAdReply: {title: set.wm, body: set.author, sourceUrl: set.web, thumbnail: set.thumb}}}
let user = db.data.users[m.sender]
    conn.slots = conn.slots ? conn.slots : {}
    if (m.chat in conn.slots) return conn.reply(m.chat, 'Masih ada Yang Melakukan Slots Disini, Tunggu Sampai selesai!!')
    else conn.slots[m.chat] = true
    try { 
        if (args.length < 1) return conn.reply(m.chat, `Gunakan format *${usedPrefix}${command} [jumlah]*
...
*${WinOrLose}* *${Hadiah}*
`, set.wm, null, [['Main Lagi', `.slot ${text}`]], m)
    } catch (e) {
        console.log(e)
