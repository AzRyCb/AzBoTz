...
let handler = async (m, { conn, usedPrefix, text, args, command }) => {
 ...
 let timers = clockString(_timers)
  ...
  if (new Date - user.lasthunt > 1200000) {
    ...
    if (player.health < 0) {
      ...
      player.health = 100
      await conn.sendButton(m.chat, msg, set.wm, null, [['Menu', `${usedPrefix}menu`],['Owner', `${usedPrefix}owner`]], m, { mentions: conn.parseMention(msg) })
      return
    }
...
+1 Tiketcoin`
    await conn.sendButton(m.chat, pesan, set.wm, null, [['Menu', `${usedPrefix}menu`],['Owner', `${usedPrefix}owner`]], m, { mentions: conn.parseMention(pesan) })
