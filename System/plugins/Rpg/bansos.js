...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    ...
    db.data.users[m.sender].lastbansos = db.data.users[m.sender].lastbansos || 0
    ...
    let __timers = (new Date - db.data.users[m.sender].lastbansos)
    let _timers = (604800000 - __timers) 
    let timers = clockString(_timers)
    let user = db.data.users[m.sender]
    if (new Date - db.data.users[m.sender].lastbansos > 300000) {
      if (Aku > Kamu) {
        ...
        user.money -= 3000000
        db.data.users[m.sender].lastbansos = new Date * 1
      } else if (Aku < Kamu) {
        user.money += 3000000
