...
let handler = (m, { conn, usedPrefix, command, text }) => {
  conn.tembak = conn.tembak || { musuh: [], tembak: [] }
   if(/kiri/i.test(text)) {
    ...
    if(conn.tembak.musuh.indexOf("🥷") === conn.tembak.tembak.indexOf("🤠")) return conn.sendButton(m.chat, pos, set.wm, [
                                                                                              ['🔫 Tembak', `${usedPrefix}koboy tembak`]             
                                                                                              ])
    return conn.sendButton(m.chat, pos, set.wm, [
