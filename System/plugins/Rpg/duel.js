...
let handler = async ( m, { conn, args, command}) => {
  ...
  let timers = clockString(_timers)
     ...
     if (/duel/.test(command)) {
       if (!who) return conn.reply(m.chat, 'tag yg ingin di ajak duel!')

     let pler = `@${m.sender.replace(/@.+/, '')} Mengajak duel ${args[0]}\n\nPilih Y Atau No`
     let mentionedJid = [m.sender]

       if (new Date - user.lastduel > 300000) {
      conn.sendButton(m.chat, pler, set.wm, null, [[`Ya`, `+dya`], [`No`, `+dno`]], m, { mentions: conn.parseMention(mentionedJid) })
