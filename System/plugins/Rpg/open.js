...
const tfinventory = {
  ...
  let imgr = set.flaaa.getRandom()
      ...
      const tfcrates = Object.keys(tfinventory.tfcrates).map(v => user[v] && `⮕ ${set.rpg.emoticon(v)} ${v}: ${user[v]}`).filter(v => v).join('\n').trim()
      let listCrate = Object.fromEntries(Object.entries(rewards).filter(([v]) => v && v in user))
      let info = `🧑🏻‍🏫 ᴜsᴇʀ: *${conn.getName(m.sender)}*
  
  🔖 ᴄʀᴀᴛᴇ ʟɪsᴛ :
  ${Object.keys(tfinventory.tfcrates).map(v => user[v] && `⮕ ${set.rpg.emoticon(v)} ${v}: ${user[v]}`).filter(v => v).join('\n')}
  –––––––––––––––––––––––––
  💁🏻‍♂ ᴛɪᴩ :
  ⮕ ᴏᴩᴇɴ ᴄʀᴀᴛᴇ:
  ...
  `.trim()
      ...
      if (!(type in listCrate)) return conn.sendButton(m.chat, `*${set.htki} OPEN CRATES ${set.htka}*`, info, imgr + 'open', [
  [`ᴄᴏᴍᴍᴏɴ`, `${usedPrefix}open common`],
  [`ᴜɴᴄᴏᴍᴍᴏɴ`, `${usedPrefix}open uncommon`]
  ], m)
      if (user[type] < count) return conn.reply(m.chat, `
  Your *${set.rpg.emoticon(type)}${type} crate* is not enough!, you only have ${user[type]} *${set.rpg.emoticon(type)}${type} crate*
  ...
  `.trim())
      ...
      user[type] -= count * 1
      conn.reply(m.chat, `
  You have opened *${count}* ${set.rpg.emoticon(type)}${type} crate and got:
  ${Object.keys(crateReward).filter(v => v && crateReward[v] && !/legendary|pet|mythic|diamond|emerald/i.test(v)).map(reward => `
  *${set.rpg.emoticon(reward)}${reward}:* ${crateReward[reward]}
  `.trim()).join('\n')}
  `.trim())
