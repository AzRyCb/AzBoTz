...
await conn.sendHydrated(m.chat, caption, set.wm, x.result, null, null, null, null, [
...
let x = await pe.json()
  await conn.sendButton(m.chat, '*Nih*', set.wm, x.url, [
...
let rep = x.score
let skor = rep * 10
let pslot = db.data.users[m.sender]
pslot.limit += skor
...
*name:* ${x.name}
*imdburl:* ${x.imdburl}`
await conn.sendButton(m.chat, caption, set.wm, x.poster, [
                ['Next', `${usedPrefix + command} ${text}`]
            ], m, { quoted: fakes })
}
...
`
await conn.sendButton(m.chat, caption, set.wm, x.color_image, [
                ['Next', `${usedPrefix + command} ${text}`]
            ], m, { quoted: fakes })
}
...
let caption = `*hex:* ${x.hex}
*name:* ${x.name}`
await conn.sendButton(m.chat, caption, set.wm, x.image, [
                ['Next', `${usedPrefix + command} ${text}`]
            ], m, { quoted: fakes })
}
...
await conn.sendButton(m.chat, caption, set.wm, x.thumbnail, [
                ['Next', `${usedPrefix + command} ${text}`]
            ], m, { quoted: fakes })
}
...
await conn.sendButton(m.chat, caption, set.wm, x.avatar, [
                ['Next', `${usedPrefix + command} ${text}`]
            ], m, { quoted: fakes })
}
...
let caption = `*Ciee...* ${toM(a)} ❤️ ${toM(b)}`
    await conn.sendButton(m.chat, caption, set.wm, img, [['jodohnya', `${usedPrefix}jodohnya`],['jodohku', `${usedPrefix}jodohku`]], m, { mentions: conn.parseMention(caption) })
}
...
let caption = `*Result:*`
await conn.sendButton(m.chat, caption, set.wm, drip, [
                ['Next', `${usedPrefix + command} ${text}`]
...
handler.command = handler.help = ['cewekracing', 'imdb', 'popss', 'popcolor', 'popcolor2', 'popsong', 'popsteam', 'sadcat', 'oogway', 'popcar', 'popgit', 'popship', 'popbiden', 'poppika', 'popmeme', 'popdrip', 'popclown', 'popuncover', 'popad', 'popblur', 'popinvert', 'poptunes', 'poptext', 'poppet', 'popalert', 'popcaution']
handler.tags = ['tools']
