...
let f = await fetch(`https://api.lolhuman.xyz/api/read-qr?apikey=${set.lolkey}&img=${url}`)
let x = await f.json()
let caption = `🤠 *Hasil:* ${x.result}`
await conn.sendHydrated(m.chat, caption, set.botdate, null, 'https://www.whatsapp.com/otp/copy/' + x.result, 'C O P Y', null, null, [
      [null, null]
    ], m)
}
...
*Percent:* ${x.percent}
*Expires:* ${x.expires}`
await conn.sendButton(m.chat, caption, set.wm, x.dest, [
...
let id = m.chat
  ...
  if (id in conn.math) {
    ...
    delete conn.math[id]
    conn.reply(m.chat, 'Hmmm...ngecheat?')
  }
  let val = text
  ...
  try {
    ...
    conn.reply(m.chat, `*${format}* = ${result}`)
  } catch (e) {
...
*Upload:* ${r.uploadDate}
*Desc:* ${r.desc}`
await conn.sendButton(m.chat, caption, set.wm, r.thumb, [
...
*Like:* ${r.lirik.result}`
await conn.sendButton(m.chat, caption, set.wm, r.img, [
...
await conn.sendButton(m.chat, caption, set.wm, xc.result.img, [
