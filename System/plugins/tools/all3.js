...
let stiker = await sticker(false, res.url, set.packname, set.author)
...
let stiker = await sticker(false, anu.result, set.packname, set.author)
conn.sendFile(m.chat, stiker, 'sticker.webp', '', m)
}
...
`
await conn.sendButton(m.chat, caption, set.wm, 'https://image.tmdb.org/t/p/w500/' + x.poster_path, [
                ['Next', `${usedPrefix + command}`]
            ], fakes, adReply)
}

if (command == 'lmaker') {
if (!args[0]) return conn.reply(m.chat, `Logo Maker List
...
1 - 6 : kgf`)
  ...
  let caption = `*⎔┉━「 ${command} 」━┉⎔*
🤠 *Query* : ${thm}`
  await conn.sendButton(m.chat, caption, set.wm, images, [
                ['Next', `${usedPrefix + command}`],
                ['Menu', `${usedPrefix}menu`]
            ], fakes, adReply)
}
...
await conn.sendButton(m.chat, caption, set.wm, x.data[0].path, [
                ['Next', `${usedPrefix + command}`]
            ], fakes, adReply)
}
...
handler.command = handler.help = ['exchange', 'ipcountry', 'emojimix3', 'emojimix4', 'calc2', 'mvsearch', 'mvsearch2', 'lmaker', 'jamdunia', 'gqr']
handler.tags = ['tools']
