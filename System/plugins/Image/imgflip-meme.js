...
let res = await fetch(`https://api.imgflip.com/caption_image?template_id=${thm}&username=Wudysoft&password=Wudysoft&text0=${text1}&text1=${text2}`)
  ...
  return conn.sendButton(m.chat, `Result from *${command}*`, set.wm, await(await fetch(x.data.url)).buffer(), [['🎀 Menu', '/menu'], ['🔄 Sticker 🔄', '.s']], fakes, adReply)
}
handler.help = ['memaker <keyword>']
handler.tags = ['update']
