...
klik wa.me/${conn.user.jid.split`@`[0]}`, m.chat, 
      ...
      if (!room.pilih) this.sendButton(room.p, `Silahkan pilih Menang +${room.poin}XP\nKalah -${room.poin_lose}XP`, set.wm, null, [[ 'Batu🗿', 'Batu'], ['Kertas📄', 'Kertas'], ['Gunting✂️', 'Gunting']], fakes, adReply)
    ...
    if (jwb && reg.test(m.text) && !room.pilih && !m.isGroup) {
      ...
      room.text = m.text
      conn.reply(m.chat, `Kamu telah memilih ${m.text} ${!room.pilih2 ? `\n\nMenunggu lawan memilih` : ''}`)
      if (!room.pilih2) this.reply(room.p2, '_Lawan sudah memilih_\nSekarang giliran kamu', 0)
    }
    if (jwb2 && reg.test(m.text) && !room.pilih2 && !m.isGroup) {
      room.pilih2 = reg.exec(m.text.toLowerCase())[0]
      room.text2 = m.text
      conn.reply(m.chat, `Kamu telah memilih ${m.text} ${!room.pilih ? `\n\nMenunggu lawan memilih` : ''}`)
