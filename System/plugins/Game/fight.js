...
conn.level = db.data.users[m.sender]
  ...
  if (typeof conn.fight[m.sender] != "undefined" && conn.fight[m.sender] == true) return conn.reply(m.chat,`*Tidak bisa melakukan pertarungan lagi karena anda sedang bertarung bro.*`)
  ...
  conn.reply(m.chat,`*Kamu* (level ${db.data.users[m.sender].level}) menantang *${conn.getName(lawan)}* (level ${db.data.users[lawan].level}) dan sedang dalam pertarungan sengit.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang.`)
