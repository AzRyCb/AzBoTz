...
export async function before(m) {
    ...
    if (!(id in this.tebakbendera))
        return this.sendButton(m.chat, 'Soal itu telah berakhir', set.wm, null, buttontebakbendera, fakes, adReply)
    if (m.quoted.id == this.tebakbendera[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebakbendera[id][3])
            delete this.tebakbendera[id]
            return this.sendButton(m.chat, '*Yah Menyerah :( !*', set.wm, null, buttontebakbendera, fakes, adReply)
        }
        let json = JSON.parse(JSON.stringify(this.tebakbendera[id][1]))
        // m.reply(JSON.stringify(json, null, '\t'))
        if (m.text.toLowerCase() == json.name.toLowerCase().trim()) {
            db.data.users[m.sender].exp += this.tebakbendera[id][2]
            this.sendButton(m.chat, `*Benar!*\n+${this.tebakbendera[id][2]} XP`, set.wm, null, buttontebakbendera, fakes, adReply)
            clearTimeout(this.tebakbendera[id][3])
            delete this.tebakbendera[id]
