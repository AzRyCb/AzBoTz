...
export async function before(m) {
    ...
    if (!(id in this.lengkapikalimat))
        return this.sendButton(m.chat, 'Soal itu telah berakhir', set.wm, null, buttonlengkapikalimat, m)
    if (m.quoted.id == this.lengkapikalimat[id][0].id) {
        ...
        if (isSurrender) {
            ...
            return this.sendButton(m.chat, '*Yah Menyerah :( !*', set.wm, null, buttonlengkapikalimat, m)
        }
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            this.sendButton(m.chat, `*Benar!*\n+${this.lengkapikalimat[id][2]} XP`, set.wm, null, buttonlengkapikalimat, m)
            clearTimeout(this.lengkapikalimat[id][3])
            delete this.lengkapikalimat[id]
