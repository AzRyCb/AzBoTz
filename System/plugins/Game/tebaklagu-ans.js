...
export async function before(m) {
    ...
    if (!(id in this.tebaklagu))
        return this.sendButton(m.chat, 'Soal itu telah berakhir', set.wm, null, buttontebaklagu, fakes, adReply)
    if (m.quoted.id == this.tebaklagu[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebaklagu[id][3])
            delete this.tebaklagu[id]
            return this.sendButton(m.chat, '*Yah Menyerah :( !*', set.wm, null, buttontebaklagu, fakes, adReply)
        }
        ...
        if (m.text.toLowerCase() == json.judul.toLowerCase().trim()) {
            ...
            this.sendButton(m.chat, `*Benar!*\n+${this.tebaklagu[id][2]} XP`, set.wm, null, buttontebaklagu, fakes, adReply)
            clearTimeout(this.tebaklagu[id][3])
            delete this.tebaklagu[id]
