let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  `.trim(), set.wm, null, buttons, fakes, adReply)
  ...
  Contoh penggunaan: ${usedPrefix}math medium
    ...
    conn.math[id] = [
        ...
        math, 4,
        setTimeout(() => {
            if (conn.math[id]) conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah ${math.result}`, set.wm, null, [['again', `${usedPrefix}${command} ${math.mode}`], ...buttons], conn.math[id][0],fakes, adReply)
            delete conn.math[id]
        }, math.time)
    ]
}
