...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  if (!m.mentionedJid[0]) return conn.reply(m.chat, `_Siapa yang ingin kamu tantang?_\nTag orangnya.. Contoh\n\n${usedPrefix}suit @${set.owner[1]}`, m.chat, { contextInfo: { mentionedJid: [set.owner[1] + '@s.whatsapp.net'] } })
...
`.trim()
  ...
  conn.suit[id] = {
    chat: await conn.sendButton(m.chat, caption, set.wm, null, [[ 'Terima', 'ok'], [ 'Tolak', 'tolak']], fakes, adReply, { mentions: conn.parseMention(caption) }),
    id: id,
    p: m.sender,
    p2: m.mentionedJid[0],
    status: 'wait',
