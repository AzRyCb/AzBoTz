...
let listSections = []
	...
	*url:* ${v.url}
	...
	listSections.push([index + ' ' + set.cmenub + ' ' + v.title, [
           ['Get Lyrics', usedPrefix + command + ' get|' + index + '|' + v.title, des]
        ]])
	})
	return conn.sendList(m.chat, set.htki + ' 📺 Genius Search 🔎 ' + set.htka, `⚡ Silakan pilih Genius Search di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`, set.wm, `☂️ Genius Search Disini ☂️`, listSections, m)
	}
...
*title_with_featured:* ${v.result.title_with_featured}
	...
	listSections.push([index + ' ' + set.cmenub + ' ' + v.result.title, [
