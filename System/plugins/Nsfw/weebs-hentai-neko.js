...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode }) => {
	...
	// ------- OTHER ------
  if (db.data.chats[m.chat].nsfw == false && m.isGroup) return conn.sendButton(m.chat, '❗ ᴏᴘᴛɪᴏɴs ɴsғᴡ ᴅɪᴄʜᴀᴛ ɪɴɪ ʙᴇʟᴜᴍ ᴅɪɴʏᴀʟᴀᴋᴀɴ ᴏʟᴇʜ ᴀᴅᴍɪɴ ɢʀᴏᴜᴘ',set.botdate, null, [['ᴇɴᴀʙʟᴇ', '.on nsfw']], m)
...
let api = `?apikey=${set.lolkey}`

//> Lolhuman
let resl = `https://api.lolhuman.xyz/api/random2/`
let apil = `?apikey=${set.lolkey}`
...
const sections = [
   {
	title: `${set.htki} N S F W ${set.htka}`,
	rows: [
...
case 'ahegao':
        if (ahegao == true) { 
	    if (db.data.users[m.sender].premiumTime < 1) return conn.sendButton(m.chat, txtprem, set.botdate, [['ʙᴜʏ ᴘʀᴇᴍɪᴜᴍ', '.premium'],['ᴏᴡɴᴇʀ', '.owner nomor']], m)
		}
