...
if (!text) throw `Gunakan contoh ${usedPrefix + command} Robot`
    let stiker = await sticker(null, API(`https://robohash.org/${text}`), set.packname, set.author)
...
if (!text) throw `Gunakan contoh ${usedPrefix + command} 6`
    ...
    await conn.sendButton(m.chat, `*Result:*`, set.wm, stiker, [
...
let urut = text.split`|`
  ...
  let teksnya2 = urut[1]
	const sections = [
    {
	title: `${set.htki} PILIH TEMA ${set.htka}`,
	rows: [
