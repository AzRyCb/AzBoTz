...
`.trim())
    if (user.pickaxe == 0) return conn.reply(m.chat, 'Mau mining ga punya pickaxe 🗿')
    if (new Date - user.lastmining <= cooldown) return conn.reply(m.chat, `
...
handler.help = handler.command = ['mining']
handler.tags = ['work']
