...
conn.level = db.data.users[m.sender]
  ...
  if (typeof conn.fightnaga[m.sender] != "undefined" && conn.fightnaga[m.sender] == true) return conn.reply(m.chat, `*Tidak bisa melakukan battle karena arena yg kamu miliki sedang kamu pakai .*`)
  ...
  conn.reply(m.chat, `*Pet Kamu* (🐱kucing ${db.data.users[m.sender].kucing}) menantang 🐈kucingnya *${conn.getName(lawan)}* (🐱kucing ${db.data.users[lawan].kucing}) lagi kelahi rebutin bini.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`)
...
handler.help = handler.command = ['fightkucing']
handler.tags = ['work']
