...
conn.level = db.data.users[m.sender]
  ...
  if (typeof conn.fightnaga[m.sender] != "undefined" && conn.fightnaga[m.sender] == true) return conn.reply(m.chat, `*Tidak bisa melakukan battle ⚔️ karena Arena yang kamu miliki dipakai untuk fight pet mu yg lain.*`)
  ...
  conn.reply(m.chat, `*Pet Kamu* (🐴centaur ${db.data.users[m.sender].centaur}) ⚔️menantang 🐴centaurnya *${conn.getName(lawan)}* (🐴centaur ${db.data.users[lawan].centaur}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`)
...
handler.help = handler.command = ['fightcentaur']
handler.tags = ['work']
