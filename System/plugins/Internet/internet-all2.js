...
await conn.sendButton(m.chat, caption, set.wm, imgr + command, [
                ['Ok', `Huuu`]
            ], m)
            }
if (args[0] == 'cat') {
...
let caption = `*Result:*`
await conn.sendButton(m.chat, caption, set.wm, res, [
...
`
await conn.sendButton(m.chat, caption, set.wm, json[0].file_url, [
                ['Next', `${usedPrefix + command} |${one}|${two}`],
                ['To Sticker', `${usedPrefix}s`]
            ], m)
