...
let res = await fetch(`https://api.lolhuman.xyz/api/ceritahoror?apikey=${set.lolkey}`)
  let sul = await res.json()
  let has = sul.result
  await conn.sendButton(m.chat, `*Judul:* ${has.title}
  *Desc:* ${has.desc}
  *Story:* ${has.story}`, set.wm, has.thumbnail, [
...
let res = await fetch(`https://api.lolhuman.xyz/api/growiki?apikey=${set.lolkey}&query=${text}`)
  let sul = await res.json()
  let has = sul.result
...
let res = await fetch(`https://api.lolhuman.xyz/api/growstocks?apikey=${set.lolkey}&query=${text}`)
  let sul = await res.json()
  let has = sul.result
  await conn.sendButton(m.chat, `*Name:* ${has.name}
  *Desc:* ${has.desc}
...
let res = await fetch(`https://api.lolhuman.xyz/api/gsearch?apikey=${set.lolkey}&query=${text}`)
  let sul = await res.json()
  let has = sul.result
...
let res = await fetch(`https://api.lolhuman.xyz/api/gsmarena?apikey=${set.lolkey}&query=${text}`)
  let sul = await res.json()
  let has = sul.result
...
`, set.wm, has.phone_image, [
                ['Next', `${usedPrefix + command}`]
            ], fakes, adReply)
}

if (command == 'jadwalbola') {
let f = await fetch(`https://api.lolhuman.xyz/api/jadwalbola?apikey=${set.lolkey}`)
let xx = await f.json()
let v = xx.result
let teks = v.map(v => {
return `
...
if (command == 'jadwaltv') {
let json = await fetch(`https://api.lolhuman.xyz/api/jadwaltv/now?apikey=${set.lolkey}`)
...
let f = await fetch(`https://api.lolhuman.xyz/api/jalantikus?apikey=${set.lolkey}`)
let xx = await f.json()
let v = xx.result
let teks = v.map(v => {
return `
...
let f = await fetch(`https://api.lolhuman.xyz/api/jaraktempuh?apikey=${set.lolkey}&kota1=${args[0]}&kota2=${args[1]}`)
let xx = await f.json()
let v = xx.result
let teks = v.map(v => {
return `
...
conn.sendMessage(m.chat, listMessage, {quoted:{key : {participant : '0@s.whatsapp.net'},message: {documentMessage: {title: set.wm,jpegThumbnail: Buffer.alloc(0)}}}})
}
