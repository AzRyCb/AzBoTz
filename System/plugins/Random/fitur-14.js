...
`
await conn.sendButton(m.chat, caption, set.wm, x.thumb, [
...
Url: ${v.url}
  ...
  await conn.sendButton(m.chat, teks, set.wm, `${v.result.img}`, [
...
if (args[0] == 'artikata') {
if (!one) return conn.reply(m.chat, `Example: ${usedPrefix + command} ${args[0]} |query`)
...
${x.url}
`.trim()
await conn.sendButton(m.chat, caption, set.wm, x.img, [
...
${x.result}
`
await conn.sendButton(m.chat, caption, set.wm, x.gambar, [
                ['Next', `${usedPrefix + command} ${args[0]}`]
            ], m, { quoted: fakes })
}
if (args[0] == 'count') {
...
let caption = `*Result:*
`
await conn.sendButton(m.chat, caption, set.wm, x.result, [
                ['Next', `${usedPrefix + command} ${args[0]}`]
            ], m, { quoted: fakes })
}
if (args[0] == 'cuaca') {
...
let caption = `*Result:*
`
await conn.sendButton(m.chat, caption, set.wm, img, [
                ['Next', `${usedPrefix + command} ${args[0]}`]
            ], m, { quoted: fakes })
}
if (args[0] == 'drakoron') {
...
gaji: ${v.gaji}
  ...
  //conn.reply(m.chat, teks)
  await conn.sendButton(m.chat, teks, set.wm, v.img, [
...
Wilayah: ${x.Wilayah}
`
await conn.sendButton(m.chat, caption, set.wm, x.Map, [
...
channel: ${v.channel}
  ...
  //conn.reply(m.chat, teks)
  await conn.sendButton(m.chat, teks, set.wm, null, [
                ['Search!', `${usedPrefix + command}`]
            ], m, { quoted: fakes })
}
if (args[0] == 'katabijak') {
...
if (args[0] == 'nik') {
if (!one) return conn.reply(m.chat, `Example: ${usedPrefix + command} ${args[0]} |12345678`)
...
tgl: ${v.tgl}
  ...
  //conn.reply(m.chat, teks)
  await conn.sendButton(m.chat, teks, set.wm, v.thumb, [
                ['Search!', `${usedPrefix + command}`]
            ], m, { quoted: fakes })
}
if (args[0] == 'otakunews') {
...
`
await conn.sendButton(m.chat, caption, set.wm, x.result.thumb, [
...
await conn.sendButton(m.chat, set.wm, set.wm, x.result.male, [
                ['Next', `${usedPrefix + command} ${args[0]}`]
            ], m, { quoted: fakes })
await conn.sendButton(m.chat, set.wm, set.wm, x.result.female, [
                ['Next', `${usedPrefix + command} ${args[0]}`]
            ], m, { quoted: fakes })
}
...
await conn.sendButton(m.chat, set.wm, set.wm, img, [
                ['Next', `${usedPrefix + command} ${args[0]}`]
            ], m, { quoted: fakes })
}
...
`
await conn.sendButton(m.chat, caption, set.wm, `${x.result[0].thumb}`, [
...
const size = yt.audio[q].fileSizeH
 ...
 await conn.reply(m.chat, `▢ Tɪᴛᴛʟᴇ: ${ttl}
▢  Sɪᴢᴇ: ${size}

▢ Ｌｏａｄｉｎｇ. . .`)
  await conn.sendFile(m.chat, dl_url, ttl + '.mp3', set.wm, m, null, {
    asDocument: false
  })
  }
  ...
  let _thumb = {}
    try { _thumb = { jpegThumbnail: set.thumb } }
    catch (e) { }
...
▢ Ｌｏａｄｉｎｇ. . .`)
  await conn.sendMessage(m.chat, { [/^(?:-|--)doc$/i.test(args[1]) || null ? 'document' : 'video']: { url: dl_url }, fileName: `${set.wm}.mp4`, mimetype: 'video/mp4', ..._thumb }, { quoted: m })
}
