...
let x = await f.json()
await conn.sendFile(m.chat, x.url, 'image.png', set.wm, m)
}
if (args[0] == '8ball') {
...
let x = await f.json()
await conn.sendButton(m.chat, `*Pencet untuk mendengar hasil:*`, set.wm, null, [
...
let x = await f.json()
if (args[0] == 'neko') {
await conn.sendFile(m.chat, x.results[0].url, 'image.png', set.wm, m)
}
if (args[0] == 'waifu') {
await conn.sendFile(m.chat, x.results[0].url, 'image.png', set.wm, m)
}
...
`
conn.sendButton(m.chat, cp, set.wm, v.picture.large, [
