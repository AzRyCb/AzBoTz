...
do b = ps.getRandom()
while (b === a)
conn.reply(m.chat, `*${toM(a)}, KAU HARUS MENIKAH ${toM(b)}, kamu MENJADI PASANGAN YANG BAIK 💓*`, null, {
mentions: [a, b]
})
}
...
return conn.reply(m.chat, 'Mampus Salah Nomor')
}
}
...
*Genre:* ${has.genre}
  ...
  *Deskripsi:* ${has.description}`, wm, has.thumb, [
...
let res = await fetch(`https://yog-apikey.herokuapp.com/api/muslim/quran?surah=${text1}&ayat=${text2}&apikey=YogGanz`)
  ...
  *Surah:* ${x.surah.tafsir.id}`, wm, null, [
...
let url = `https://api.lolhuman.xyz/api/meme/memeindo?apikey=${set.lolkey}`
await conn.sendHydrated(m.chat, caption, set.wm, url, null, null, null, null, [
...
let caption = `Nihh banh ${command} nya`
let url = `https://api.lolhuman.xyz/api/random/meme?apikey=${set.lolkey}`
await conn.sendHydrated(m.chat, caption, set.wm, url, null, null, null, null, [
      ['Meme EN', usedPrefix + 'meme'],
      ['Meme ID', usedPrefix + 'memeindo'],
...
let caption = `Nihh banh ${command} nya`
let url = `https://api.lolhuman.xyz/api/meme/darkjoke?apikey=${set.lolkey}`
await conn.sendHydrated(m.chat, caption, set.wm, url, null, null, null, null, [
...
*/
if (command == 'beasiswa') {
            let gas = await fetch(`https://api.lolhuman.xyz/api/indbeasiswa?apikey=${set.lolkey}`)
...
if (command == 'stimker') {
            ...
            let lis = [apilol + "anjing?apikey=" + set.lolkey,
            apilol + "patrick?apikey=" + set.lolkey,
            apilol + "amongus?apikey=" + set.lolkey,
            apilol + "gawrgura?apikey=" + set.lolkey,
            apilol + "bucinstick?apikey=" + set.lolkey
            ]
            let lisn = ["🐕‍🦺 anjing",
            "⭐ patrick",
            "💩 amongus",
		...
		title: set.htjava + ' ' + lisn[v] + ' Sticker',
		description: 'By ' + wm,
...
let f = await fetch(`https://api.lolhuman.xyz/api/apkdownloader?apikey=${set.lolkey}&package=${text}`)
let x = await f.json()
let caption = `*Apk Name:* ${x.result.apk_name}
*Version:* ${x.result.apk_version}
*Author:* ${x.result.apk_author}
`
await conn.sendHydrated(m.chat, caption, set.wm, x.result.apk_icon, x.result.apk_link, 'Link', null, null, [
      ['Download', `${usedPrefix + command}`],
      ['HostApk', usedPrefix + 'hostapk'],
      ['Menu', usedPrefix + 'menu']
    ], m, { quoted: fakes })
    await conn.reply(m.chat, 'File dikirim..')
...
let f = await fetch(`https://api.lolhuman.xyz/api/proxysite?apikey=${set.lolkey}&url=${text}`)
let x = await f.json()
let caption = `🤠 *Country:* ${x.result}`
conn.sendButton(m.chat, caption, set.wm, null, [
...
if (!args[0]) throw `Contoh penggunaan ${usedPrefix}${command} https://www.mirrored.to/files/EB7BOJU3/[NekoPoi]_Isekai_Harem_Monogatari_-_04_[720P][nekopoi.care].mp4_links`
        ...
        let f = await fetch(`https://api.lolhuman.xyz/api/mirrorcreator?apikey=${set.lolkey}&url=${text}`)
...
if (!args[0]) throw `Contoh penggunaan ${usedPrefix}${command} https://ouo.io/8BgQ1w`
        ...
        let f = await fetch(`https://api.lolhuman.xyz/api/ouo?apikey=${set.lolkey}&url=${text}`)
let jsons = await f.json()
let x = jsons.result
...
if (!args[0]) throw `Contoh penggunaan ${usedPrefix}${command} https://google.com`
        ...
        let f = await fetch(`https://api.lolhuman.xyz/api/ouoshortlink?apikey=${set.lolkey}&url=${text}`)
let jsons = await f.json()
let x = jsons.result
let caption = `*Result:* ${x}`
        conn.sendButton(m.chat, caption, set.wm, null, [
...
if (!args[0]) throw `Contoh penggunaan ${usedPrefix}${command} https://google.com`
        ...
        let f = await fetch(`https://api.lolhuman.xyz/api/shortlink?apikey=${set.lolkey}&url=${text}`)
let jsons = await f.json()
let x = jsons.result
let caption = `*Result:* ${x}`
        conn.sendButton(m.chat, caption, set.wm, null, [
...
if (!args[0]) throw `Contoh penggunaan ${usedPrefix}${command} https://google.com`
        ...
        let f = await fetch(`https://api.lolhuman.xyz/api/shortlink2?apikey=${set.lolkey}&url=${text}`)
let jsons = await f.json()
let x = jsons.result
let caption = `*Result:* ${x}`
        conn.sendButton(m.chat, caption, set.wm, null, [
...
if (!args[0]) throw `Contoh penggunaan ${usedPrefix}${command} https://google.com`
        ...
        let f = await fetch(`https://api.lolhuman.xyz/api/shortlink3?apikey=${set.lolkey}&url=${text}`)
let jsons = await f.json()
let x = jsons.result
let caption = `*Result:* ${x}`
        conn.sendButton(m.chat, caption, set.wm, null, [
...
if (!args[0]) throw `Contoh penggunaan ${usedPrefix}${command} https://google.com`
        ...
        let f = await fetch(`https://api.lolhuman.xyz/api/shortlink4?apikey=${set.lolkey}&url=${text}`)
let jsons = await f.json()
let x = jsons.result
let caption = `*Result:* ${x}`
        conn.sendButton(m.chat, caption, set.wm, null, [
...
let row = Object.keys(lis).map((v, index) => ({
		title: set.htjava + ' ' + lis[v] + ' Sticker',
		description: 'By ' + set.wm,
...
if (command == 'sl') {
  let res = await fetch('https://api.lolhuman.xyz/api/' + args[0] + '?apikey=' + set.lolkey + '&url=' + args[1])
  let json = await res.json()
  conn.reply(m.chat, json.result)
	}
}
...
handler.command = handler.help = ['jadian2', 'menikah', 'metercinta', 'bertanya', 'mencerpen', 'memeindo', 'randommeme', 'memedarkjoke', 'beasiswa', 'apkdown', 'proxysite', 'mirrorcreator', 'ouo', 'ouoshort', 'shortlink', 'shortlink2', 'shortlink3', 'shortlink4', 'tenor', 'sl']
handler.tags = ['random']
