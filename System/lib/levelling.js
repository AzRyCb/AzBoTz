...
const { __dirname } = (await import('./helper.js')).default 
...
export function levelup(teks, level) {
    return new Promise(async (resolve, reject) => {
        ...
        const [_spawnprocess, ..._spawnargs] = [...(global.set.support.gm ? ['gm'] : global.set.support.magick ? ['magick'] : []),
            'convert',
            xtsx,
            '-font',
            fontTexts,
...
export function xpRange(level, multiplier = global.set.multiplier || 1) {
    if (level < 0)
...
export function findLevel(xp, multiplier = global.set.multiplier || 1) {
    if (xp === Infinity)
        return Infinity
    if (isNaN(xp))
        return NaN
...
export function canLevelUp(level, xp, multiplier = global.set.multiplier || 1) {
    if (level < 0)
        return false
    if (xp === Infinity)
        return true
