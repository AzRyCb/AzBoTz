...
import {existsSync, watchFile, unwatchFile, readFileSync} from 'fs'
...
import { webp2mp4, webp2png, toAudio, toPTT  } from './sistem.js'
...
export default new class Function {
   ...
   fetchBuffer = async (PATH) => {
      return new Promise(async (resolve, reject) => {
         let data = Buffer.isBuffer(PATH) ? PATH : PATH instanceof ArrayBuffer ? PATH.toBuffer() : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? await (await fetch(PATH)).buffer() : existsSync(PATH) ? (readFileSync(PATH)) : typeof PATH === 'string' ? PATH : Buffer.alloc(0)  
	 resolve(data)
      })
   }
