...
switch (template) {
        ...
        let ixnnn = `*Result:*\n${ixn.image}`
        await conn.sendButtonImg(m.chat, ixn.image, ixnnn, wm, 'Video', '.kay img |meme', fakes, adReply)
        if (one == 'meme') {
        let gnn = `*Result:*\n${ixn.video}`
        await conn.sendButtonImg(m.chat, ixn.video, gnn, wm, 'To Sticker', '.s', fakes, adReply)
        }
            break
            case 'vid':
        ...
        let vxnnn = `*Result:*\n${vxn.video}`
        await conn.sendButtonImg(m.chat, vxn.video, vxnnn, wm, 'To Sticker', '.s', fakes, adReply)
            break
            case 'txt':
        ...
        if (one == 'asmaulhusna') {
        let ttx = `*Result:*\n${txn.number}\n${txn.latin}\n${txn.arab}\n${txn.translate_id}\n${txn.translate_en}\n`
        await conn.sendButtonImg(m.chat, logo, ttx, wm, 'M E N U', '.menu', fakes, adReply)
        }
        if (one == 'quotes') {
        let tpx = `*Result:*\n${txn.quotes}\n${txn.wm}\n`
        await conn.sendButtonImg(m.chat, logo, tpx, wm, 'M E N U', '.menu', fakes, adReply)
        }
            break
            case 'tpro':
        let vpx = `https://api-kaysa.herokuapp.com/api/textpro/${one}?text=${two}&apikey=ltQh6BQN`
        let vpnn = `*Result:*\n`
        await conn.sendButtonImg(m.chat, vpx, vpnn, wm, 'To Sticker', '.s', fakes, adReply)
            break
