...
let handler = async (m, { conn, command, usedPrefix }) => {
    ...
    if (id in conn.tebakanjime) {
        conn.sendButton(m.chat, 'Masih ada soal belum terjawab di chat ini', set.wm, null, buttons, conn.tebakanjime[id][0], fakes, adReply)
        throw false
    }
...
Bonus: ${poin} XP
    `.trim()
    conn.tebakanjime[id] = [
        await conn.sendButton(m.chat, caption, set.wm, json.image, buttons, fakes, adReply),
        json, poin,
        setTimeout(() => {
            if (conn.tebakanjime[id]) conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.name}*\n*Desk:* ${json.desc}`, set.wm, null, [
                ['tebakanime', '/tebakanime']
            ], conn.tebakanjime[id][0], fakes, adReply)
