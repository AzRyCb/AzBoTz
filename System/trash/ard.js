...
try {
	if(command) {
			switch(template) {
				...
				case 'trapnime':
					...
					try {
						let bd = await sticker(null, bc.result, set.packname, set.author)
  ...
  let ee_ = new Sticker(cc_, { pack: set.packname, author: set.author, type: StickerTypes.FULL })
  let dd_
  try {
  ...
  let eee_ = new Sticker(ccc_, { pack: set.packname, author: set.author, type: StickerTypes.FULL })
  let ddd_
  try {
